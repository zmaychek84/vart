...
Pool<DPUVersion::DPUV2>::Pool(int inst_type, int instid,
  ...
  if (pool_type_ == Calc::POOL_TYPE_MAX_REDUCE) {
  ...
  if (pool_type_ == Calc::POOL_TYPE_MAX_POOL ||
      pool_type_ == Calc::POOL_TYPE_MAX_REDUCE) {
    std::fill(rlt_dtype_.begin(), rlt_dtype_.end(), MIN_DTYPE_VALUE);
  } else if (pool_type_ == Calc::POOL_TYPE_AVG_POOL) {
...
Pool<DPUVersion::DPUV4E>::Pool(int inst_type, int instid,
  ...
  bank_addr_in_2_ = field_val_[TABLE::POOL_FIELD_BANK_ADDR_IN_2];
  bank_addr_in_3_ = field_val_[TABLE::POOL_FIELD_BANK_ADDR_IN_3];
  valid_pixel_parallel_ = field_val_[TABLE::POOL_FIELD_VALID_PIXEL_PARALLEL];
  shift_cut_ = field_val_[TABLE::POOL_FIELD_SHIFT_CUT];
  ...
  stride_out_ = POOLINIT::CUROBJ->get_stride_out();
  jump_read_ = POOLINIT::CUROBJ->get_jump_read();
  jump_read_endl_ = POOLINIT::CUROBJ->get_jump_read_endl();
  jump_write_ = POOLINIT::CUROBJ->get_jump_write();
  jump_write_endl_ = POOLINIT::CUROBJ->get_jump_write_endl();
  kernel_h_ = POOLINIT::CUROBJ->get_kernel_h();
  kernel_w_ = POOLINIT::CUROBJ->get_kernel_w();
  pool_type_ = POOLINIT::CUROBJ->get_pool_type();
  multi_factor_ = POOLINIT::CUROBJ->get_multi_factor();
...
void Pool<dv>::pad() {
  ...
  if (pool_type_ == Calc::POOL_TYPE_MAX_REDUCE) return;

  auto pad_value = pool_type_ == Calc::POOL_TYPE_MAX_POOL ? MIN_DTYPE_VALUE : 0;
  // padding left
...
void Pool<T>::target_check() {
  // bank access
  /// pool in
  auto access_white_list =
      ArchCfg::Instance().get_bank_access_white_list("pool-in");
  ...
  /// pool out
  access_white_list =
      ArchCfg::Instance().get_bank_access_white_list("pool-out");
  UNI_LOG_CHECK((access_white_list.count(bank_id_out_) > 0), SIM_OUT_OF_RANGE)
      << "bank_id_out (= " << bank_id_out_ << ") does not match target!"
      << endl;
  // pool type
  auto support_list = ArchCfg::Instance().get_param().pool_engine().pool_type();
  if (ArchCfg::Instance().get_param().pool_engine().pool_type_size() == 0)
    support_list.Add(vitis::ai::Target_Pool::max);
  UNI_LOG_CHECK((((pool_type_ == Calc::POOL_TYPE_MAX_POOL) &&
                  (std::count(support_list.begin(), support_list.end(),
                              vitis::ai::Target_Pool::max) > 0)) ||
                 ((pool_type_ == Calc::POOL_TYPE_AVG_POOL) &&
                  (std::count(support_list.begin(), support_list.end(),
                              vitis::ai::Target_Pool::avg) > 0)) ||
                 ((pool_type_ == Calc::POOL_TYPE_MAX_REDUCE) &&
                  (std::count(support_list.begin(), support_list.end(),
                              vitis::ai::Target_Pool::max_reduce) > 0))),
                SIM_PARAMETER_FAILED)
      << "Pool type = " << pool_type_ << " is unsupported on "
      << ArchCfg::Instance().get_param().name() << endl;
  if (pool_type_ == Calc::POOL_TYPE_MAX_REDUCE) return;
  // avg_limit
  /// kernel size
  string limit_type =
      (pool_type_ == Calc::POOL_TYPE_AVG_POOL) ? "pool-avg" : "pool-max";
  auto limit_white_list =
      ArchCfg::Instance().get_instr_limit_white_list(limit_type + "-kernel");
  ...
  /// stride
  limit_white_list =
      ArchCfg::Instance().get_instr_limit_white_list(limit_type + "-stride");
...
template <>
void Pool<DPUVersion::DPUV4E>::pool_one(int idx_src_h, int idx_src_w) {
  // init result value
  int idx_dst_h = idx_src_h / stride_h_;
  int idx_dst_w = idx_src_w / stride_w_;
