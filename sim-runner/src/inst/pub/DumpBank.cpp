...
std::set<DDRblock> DDRblock::usedBlocks;
std::string DDRblock::subgraphName_ = "";
bool DDRblock::status = true;

std::set<Bankblock> Bankblock::usedBlocks_DDR2bank;
std::set<Bankblock> Bankblock::usedBlocks_engine2bank;
std::string Bankblock::subgraphName_ = "";
bool Bankblock::status_DDR2bank = true;
bool Bankblock::status_engine2bank = true;
...
template <>
DumpBank<DPUVersion::DPUV4E>::DumpBank(int inst_type, int instid,
                                       vector<string>& inst_str,
                                       vector<uint32_t>& inst_val)
    : InstBase(inst_type, instid, inst_str, inst_val) {
  debug_ = debug_ && SimCfg::Instance().get_debug_instr(SimCfg::DBG_INSTR_DUMP);
...
template <>
DumpBank<DPUVersion::XV2DPU>::DumpBank(int inst_type, int instid,
                                       vector<string>& inst_str,
                                       vector<uint32_t>& inst_val)
    : InstBase(inst_type, instid, inst_str, inst_val) {
  debug_ = debug_ && SimCfg::Instance().get_debug_instr(SimCfg::DBG_INSTR_DUMP);
  ...
  bank_start_ = field_val_[TABLE::DUMPBANK_FIELD_BANK_START];
  bank_dump_num_ = field_val_[TABLE::DUMPBANK_FIELD_BANK_NUM];
  UNI_LOG_CHECK(bank_dump_num_ <= 1, SIM_OUT_OF_RANGE)
      << "bank_dump_num: " << bank_dump_num_ << " is out of range";
}

template <>
DumpBank<DPUVersion::XV3DPU>::DumpBank(int inst_type, int instid,
                                       vector<string>& inst_str,
                                       vector<uint32_t>& inst_val)
    : InstBase(inst_type, instid, inst_str, inst_val) {
  debug_ = debug_ && SimCfg::Instance().get_debug_instr(SimCfg::DBG_INSTR_DUMP);
