...
ReadInst::ReadInst(const vector<string>& inst_vec) {
  ...
  } else if (isa_version == int(DPUVersion::XV2DPU)) {
    Helper<DPUVersion::XV2DPU>::ReadInstInit(minus_mask, field_mask);
    for (auto& inst : inst_vec) {
      vector<string> inst_str;
      vector<uint32_t> inst_val;
      Helper<DPUVersion::XV2DPU>::GenInst(inst, inst_fmt_, minus_mask,
                                           field_mask, inst_str, inst_val,
                                           v_inst_mc_);
      v_inst_str_.emplace_back(inst_str);
      v_inst_val_.emplace_back(inst_val);
    }
  } else if (isa_version == int(DPUVersion::XV3DPU)) {
    Helper<DPUVersion::XV3DPU>::ReadInstInit(minus_mask, field_mask);
    for (auto& inst : inst_vec) {
      vector<string> inst_str;
      vector<uint32_t> inst_val;
      Helper<DPUVersion::XV3DPU>::GenInst(inst, inst_fmt_, minus_mask,
                                          field_mask, inst_str, inst_val,
                                          v_inst_mc_);
      v_inst_str_.emplace_back(inst_str);
      v_inst_val_.emplace_back(inst_val);
