...
template <DPUVersion dv>
deque<int> ElewInit<dv>::jump_bank_;
template <DPUVersion dv>
deque<int> ElewInit<dv>::share_pp_;
template <DPUVersion dv>
deque<int> ElewInit<dv>::macc_dim_;
...
ElewInit<DPUVersion::DPUV2>::ElewInit(int inst_type, int inst_id,
  ...
  local_share_pp_ = field_val_[TABLE::ELEWINIT_FIELD_SHARE_PP];
  local_jump_bank_ = field_val_[TABLE::ELEWINIT_FIELD_JUMP_BANK];
  ...
  share_pp_.push_back(local_share_pp_);
  jump_bank_.push_back(local_jump_bank_);
}
...
template <DPUVersion dv>
int ElewInit<dv>::get_jump_bank(int i) {
  auto jump_bank = jump_bank_.front();
  jump_bank_.pop_front();
  return jump_bank;
}

template <DPUVersion dv>
int ElewInit<dv>::get_share_pp(int i) {
  auto share_pp = share_pp_.front();
  share_pp_.pop_front();
  return share_pp;
}

template <DPUVersion dv>
int ElewInit<dv>::get_macc_dim(int i) {
  auto macc_dim = macc_dim_.front();
  macc_dim_.pop_front();
  return macc_dim;
}
...
template class ElewInit<DPUVersion::XV2DPU>;
template class ElewInit<DPUVersion::XV3DPU>;
