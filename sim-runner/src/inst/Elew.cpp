...
Elew<DPUVersion::DPUV2>::Elew(int inst_type, int inst_id,
  ...
  if (elew_type_ == Calc::ELEW_TYPE_MULT) {
    UNI_LOG_CHECK(num_ == 2, SIM_PARAMETER_FAILED)
        << " when elew_type_ == 1, do mult, only support num = 2, but" << num_
        << " is given";
  }
  ...
  for (int i = 0; i < num_; i++) {
    ...
    share_pp_[i] = ELEWINIT::get_share_pp(i);
    jump_bank_[i] = ELEWINIT::get_jump_bank(i);
  }
  ...
  for (auto idx = 0; idx < num_; idx++)
    UNI_LOG_CHECK(jump_bank_[idx] * vpp_ <= pp_conv_, SIM_PARAMETER_FAILED)
        << "jump_bank(" << jump_bank_[idx] << ") * vpp(" << pp
        << ") must be no more than bank_num inside one image bank group ("
        << pp_conv_ << ").";
  cg_ = channel_group_;
...
Elew<DPUVersion::DPUV4E>::Elew(int inst_type, int inst_id,
  ...
  hsigmoid_in_ = field_val_[TABLE::ELEW_FIELD_HSIGMOID_IN];
  shift_hsigmoid_ = field_val_[TABLE::ELEW_FIELD_SHIFT_HSIGMOID];
...
template <>
void Elew<DPUVersion::DPUV2>::read() {
  for (int i = 0; i < src_h_; i++) {
    auto bank_depth =
        Buffer<DPU_DATA_TYPE>::Instance().GetBank(bank_id_in_[i])->GetH();
    for (int j = 0; j < src_w_; j++) {
      for (int k = 0; k < vpp_; k++) {
        int bank_id = bank_id_in_[i] + k * jump_bank_[i];
        int bank_id_base = bank_id_in_[i] / pp_conv_ * pp_conv_;
        int wrap_back = (bank_id - bank_id_base) / pp_conv_;
        auto bank_id_this_row = bank_id % pp_conv_ + bank_id_base;
        bank_id_this_row = share_pp_[i] ? bank_id_in_[i] : bank_id_this_row;
        ...
        int jump_read = jump_read_[i];
        int bank_addr = bank_addr_in_[i];
        bank_addr += share_pp_[i] ? 0 : wrap_back * jump_read_endl_[i];
        bank_addr += j * jump_read;
...
template <>
void Elew<DPUVersion::XVDPU>::read() {
  for (int i = 0; i < src_h_; i++) {
    auto bank_depth =
        Buffer<DPU_DATA_TYPE>::Instance().GetBank(bank_id_in_[i])->GetH();
    for (int j = 0; j < src_w_; j++) {
...
template <DPUVersion T>
void Elew<T>::target_check() {
  // bank access
  /// elew in
  auto access_white_list =
      ArchCfg::Instance().get_bank_access_white_list("elew-in");
  for (int i = 0; i < num_; i++)
    UNI_LOG_CHECK((access_white_list.count(bank_id_in_[i]) > 0),
                  SIM_OUT_OF_RANGE)
        << "bank_id_in[" << i << "] (= " << bank_id_in_[i]
        << ") does not match target!" << endl;
  access_white_list.clear();
  /// elew out
  access_white_list =
      ArchCfg::Instance().get_bank_access_white_list("elew-out");
  ...
  // elew type check
  support_list.Clear();
  support_list = ArchCfg::Instance().get_param().eltwise_engine().elew_type();
  if (ArchCfg::Instance().get_param().eltwise_engine().elew_type_size() == 0)
    support_list.Add(vitis::ai::Target_Eltwise::add);
  UNI_LOG_CHECK((((elew_type_ == Calc::ELEW_TYPE_ADD) &&
                  (std::count(support_list.begin(), support_list.end(),
                              vitis::ai::Target_Eltwise::add) > 0)) ||
                 ((elew_type_ == Calc::ELEW_TYPE_MULT) &&
                  (std::count(support_list.begin(), support_list.end(),
                              vitis::ai::Target_Eltwise::mult) > 0))),
                SIM_PARAMETER_FAILED)
      << "Eltwise type = " << elew_type_ << " is unsupported on "
      << ArchCfg::Instance().get_param().name() << endl;
}
...
void Elew<T>::calc() {
  ...
  auto shift_write_bound = pow(2, shift_write_bit_width - 1);
    ...
    for (int i = 0; i < src_w_; i++) {
      for (int j = 0; j < ic_; j++) {
        ...
        for (int k = 0; k < src_h_; k++) {
          ...
          if (elew_type_ == Calc::ELEW_TYPE_ADD) {
            tmp += floor((double)data_[data_addr] * 4.0 / shift_read_factor[k]);
          } else if (elew_type_ == Calc::ELEW_TYPE_MULT) {
            tmp *= floor((double)data_[data_addr] * 4.0 / shift_read_factor[k]);
          }
        }
        tmp *= shift_write_factor;
        tmp /= (elew_type_ == Calc::ELEW_TYPE_ADD) ? 4.0 : 16.0;
...
template <>
void Elew<DPUVersion::DPUV2>::debug_input_tick() {
  if (!debug_) return;
      ...
      for (auto idx_num = 0; idx_num < num_; idx_num++) {
        ...
        for (auto idx_vpp = 0; idx_vpp < vpp_; idx_vpp++) {
          auto ori_offset =
              ...
              idx_cg * cp_;
          auto bank_id = share_pp_[idx_num]
                             ? bank_id_in_[idx_num]
                             : (bank_id_base + (bank_id_in_[idx_num] +
                                                idx_vpp * jump_bank_[idx_num]) %
                                                   pp_conv_);

          auto wrap_back = ((bank_id_in_[idx_num] - bank_id_base +
                             idx_vpp * jump_bank_[idx_num]) /
                            pp_conv_);
          auto bank_addr = bank_addr_in_[idx_num];
          bank_addr +=
              share_pp_[idx_num] ? 0 : (wrap_back * jump_read_endl_[idx_num]);
          bank_addr += (idx_w * jump_read_[idx_num] + idx_cg);
          bank_addr %= bank_depth;
...
template <>
void Elew<DPUVersion::XVDPU>::debug_input_tick() {
  if (!debug_) return;
