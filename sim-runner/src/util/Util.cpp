...
void Util::TimeBegin(struct timeval& begin) { gettimeofday(&begin, 0); }

unsigned long Util::TimeEnd(const struct timeval& begin, int type) {
  struct timeval end;
  gettimeofday(&end, 0);
  if (type == TIME_S) {
...
void Util::DelFileOrDir(const string& name) {
  ...
  if (lstat(fname, &buf) == -1) {
    UNI_LOG_FATAL(SIM_FAIL_OPEN_FAILED)
...
uint64_t Util::LoadBin(const string& load_name, char* data, uint64_t size) {
  ...
  UNI_LOG_CHECK(fsize <= size, SIM_OUT_OF_RANGE)
...
uint64_t Util::LoadHexContSmallEnd(const string& load_name, char* data,
  ...
  for (auto i = 0U; i < line_vec.size(); i++) {
    ...
    UNI_LOG_CHECK(line_vec[i].size() == (size_t)2 * line_size,
...
uint64_t Util::LoadHexContBigEnd(const string& load_name, char* data,
                                 uint64_t size, int line_size) {
  string line;
  std::fstream f(load_name);
  vector<string> line_vec;
...
uint64_t Util::LoadHexContSmallEndBankAddr(const string& load_name, char* data,
                                           uint64_t size, int line_size) {
  string line;
  std::fstream f(load_name);
  vector<string> line_vec;
...
uint64_t Util::LoadHexContBigEndBankAddr(const string& load_name, char* data,
                                         uint64_t size, int line_size) {
  string line;
  std::fstream f(load_name);
  vector<string> line_vec;
...
uint64_t Util::LoadHexContSmallEndDDRAddr(const string& load_name, char* data,
                                          uint64_t size, int line_size) {
  string line;
  std::fstream f(load_name);
  vector<string> line_vec;
  ...
  // every line has line_size bytes
  auto nlines =
      (size / line_size) > line_vec.size() ? line_vec.size() : size / line_size;
...
uint64_t Util::LoadHexContBigEndDDRAddr(const string& load_name, char* data,
                                        uint64_t size, int line_size) {
  string line;
  std::fstream f(load_name);
  vector<string> line_vec;
...
void Util::SaveBin(const string& save_name, const char* data, uint64_t size,
                   int mode) {
  // write into file
  DumpOfstream f;
...
void Util::SaveHexContSmallEnd(const string& save_name, const char* data,
...
void Util::SaveHexContBigEnd(const string& save_name, const char* data,
                             uint64_t size, int line_size, int mode) {
  // write into file
  DumpOfstream f;
...
void Util::SaveHexContSmallEndBankAddr(const string& save_name,
                                       const char* data, uint64_t size,
                                       int line_size, int bank_height,
                                       int bank_id_base, int bank_addr_base,
                                       int mode) {
  // write into file
  DumpOfstream f;
...
void Util::SaveHexContBigEndBankAddr(const string& save_name, const char* data,
                                     uint64_t size, int line_size,
                                     int bank_height, int bank_id_base,
                                     int bank_addr_base, int mode) {
  // write into file
  DumpOfstream f;
...
void Util::SaveHexContSmallEndDDRAddr(const string& save_name, const char* data,
                                      uint64_t size, int line_size,
                                      uint64_t addr_offset, int reg_id,
                                      int mode) {
  // open file
  DumpOfstream f;
  ...
  for (int i = 0; i < line_num; i++) {
    ...
    f << dec << reg_id << "-0x" << hex << setw(16) << setfill('0') << addr
      << " : ";
...
void Util::SaveHexContBigEndDDRAddr(const string& save_name, const char* data,
                                    uint64_t size, int line_size,
                                    uint64_t addr_offset, int reg_id,
                                    int mode) {
  // open file
...
float Util::Float2Bfloat(float value) {
  uint32_t itmp;
  std::memcpy(&itmp, &value, sizeof(uint32_t));
  ...
  itmp &= 0xFFFF0000;
  float ret;
  std::memcpy(&ret, &itmp, sizeof(float));
  return ret;
}
...
string Util::SplitFirst(const string& str, const string& delim,
                        bool trim_flag) {
