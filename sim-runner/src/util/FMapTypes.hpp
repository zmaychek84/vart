...
struct AddrAttr {
  ...
  void PrintCfg(const string& head = "addr") {
    UNI_LOG_INFO << head << ": ["
                 << "location: " << location << ", ff_mode: " << ff_mode
                 << ", ff_shift: " << ff_shift << ", reg: " << reg
                 << ", off: 0x" << std::hex << off << "(" << std::dec << off
                 << ")]" << endl;
  }

  void Check() {
    UNI_LOG_CHECK(reg >= 0 && reg < 10, SIM_OUT_OF_RANGE)
        << ": error reg value " << reg << endl;
  }
};
...
struct FMapAttr {
  ...
  void Check() {
    UNI_LOG_CHECK(h > 0 && hs > 0 && hs >= h && w > 0 && ws > 0 && ws >= w &&
                      c > 0 && cs > 0 && cs >= c,
...
struct KernelAttr {
  ...
  void PrintCfg(const string& head = "kernel") {
    UNI_LOG_INFO << head << ": ["
                 ...
                 << ", dilation: " << dilation << ", h: " << h << ", w: " << w
                 << "]" << endl;
  }
  void Check() {
    UNI_LOG_CHECK(ori_h > 0 && ori_h < 16 && ori_w > 0 && ori_w < 16 &&
                      dilation >= 1 && h > 0 && h < 16 && w > 0 && w < 16,
                  SIM_PARAMETER_FAILED)
        << ": error kernel param "
        << "ori_h = " << ori_h << ", ori_w = " << ori_w
        << ", dilation = " << dilation << ", h = " << h << ", w = " << w
