...
DWConvFix<DType, WType>::DWConvFix(const xir::Subgraph* subg, const xir::Op* op,
  ...
  if (nonlinear_type_str == "NONE" || nonlinear_type_str == "") {
    ...
    auto prelu_in_ = (int8_t)(uint8_t)op->get_attr<int>("prelu_in");
    auto prelu_shift_ = op->get_attr<int>("prelu_shift");
    prelu_alpha_ = ((double)prelu_in_) / pow(2.0, prelu_shift_);
    nonlinear_type_ = 2;
  } else if (nonlinear_type_str == "LEAKYRELU") {
    if (CPUOPBase::xir_op_->has_attr("LEAKYRELU_alpha")) {
      prelu_alpha_ = (double)(CPUOPBase::xir_op_->get_attr<float>("LEAKYRELU_alpha"));
    }
    nonlinear_type_ = 3;
...
template <typename DType, typename WType>
void DWConvFix<DType, WType>::run() {
  this->dwconv();
  fix();
}

template <typename DType, typename WType>
void DWConvFix<DType, WType>::print_param() {
  DWConv<DType, WType>::print_param();
...
template <typename DType, typename WType>
void DWConvFix<DType, WType>::fix() {
...
INSTANTIATE_TPCLASS_SPECIFIED_WEIGHTS(DWConvFix);
REG_SPECIFIED_WEIGHTS_DATATYPE_OP_INSTANCE_FUNC("depthwise-conv2d-fix", DWConvFix, "weights");
