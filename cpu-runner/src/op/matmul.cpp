...
const vector<string> Matmul<DType1, DType2, BType>::ITName = {"input", "weights", "bias"};

// constructor and deconstructor
template <typename DType1, typename DType2, typename BType>
Matmul<DType1, DType2, BType>::Matmul(const xir::Subgraph* subg,
  ...
  has_wts_ = xir_op_->get_input_num(ITName[WEIGHT]) > 0 ? true : false;
  if (has_wts_) {
    fmap_i_.emplace_back(xir_op_->get_input_tensor(ITName[WEIGHT], 0)->get_shape());
  }
...
template<typename DType1, typename DType2, typename BType>
void Matmul<DType1, DType2, BType>::run() {
...
template<typename DType1, typename DType2, typename BType>
void Matmul<DType1, DType2, BType>::print_param() {
...
template<typename DType1, typename DType2, typename BType>
void Matmul<DType1, DType2, BType>::check_param() {
...
template<typename DType1, typename DType2, typename BType>
void Matmul<DType1, DType2, BType>::read() {
  // read input data
  auto* data_ina_tb = inputs_.at(ITName[INPUT]).at(0);
  auto* data_inb_tb = has_wts_ ? inputs_.at(ITName[WEIGHT]).at(0) :
                                inputs_.at(ITName[INPUT]).at(1);
  data_ina_ = GET_CPUTB_DType_PTR(DType1, data_ina_tb);
  data_inb_ = GET_CPUTB_DType_PTR(DType2, data_inb_tb);
  ...
  if (has_bias_) {
    ...
    data_bias_ = GET_CPUTB_DType_PTR(BType, data_bias_tb);
  }

  data_out_ = GET_CPUTB_DType_PTR(DType1, output_);
}

template<typename DType1, typename DType2, typename BType>
uint64_t Matmul<DType1, DType2, BType>::get_workload() {
  // uint64_t mkn = 2 * M_ * K_ * N_;
  // uint64_t batch_out = fmap_o_.num() / M_ / N_; 
  // return batch_out * mkn;
  // coverity: Unintentional integer overflow
  return 2*K_*fmap_o_.num();
}

template<typename DType1, typename DType2, typename BType>
void Matmul<DType1, DType2, BType>::calc_param() {
  if (transpose_a_) {
...
INSTANTIATE_TPCLASS_MATMUL(Matmul);

REG_SPECIFIED_MATMUL_DATATYPE_OP_INSTANCE_FUNC("matmul", Matmul);
