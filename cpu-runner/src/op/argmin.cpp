...
const vector<string> ArgMin<DType, OType>::ITName = {
  "input",
};

// constructor and deconstructor
template <typename DType, typename OType>
ArgMin<DType, OType>::ArgMin(const xir::Subgraph* subg, const xir::Op* op,
...
template <typename DType, typename OType>
void ArgMin<DType, OType>::print_param() {
...
template <typename DType, typename OType>
void ArgMin<DType, OType>::check_param() {
...
template <typename DType, typename OType>
uint64_t ArgMin<DType, OType>::get_workload() {
  // TODO
  return 0;
}

template <typename DType, typename OType>
void ArgMin<DType, OType>::argmin() {
  int left = 1, right = 1;
  ...
  for (auto i = 0; i < left; i++)
    for (auto j = 0; j < right; j++) {
      DType min = *(img_ + i * fmap_i_[axis_] * right + j);
      rlt_[i * 1 * right + j] = 0;
...
INSTANTIATE_TPCLASS_SPECIFIED(ArgMin);
REG_SPECIFIED_DATATYPE_OP_INSTANCE_FUNC("argmin", ArgMin);
REG_SPECIFIED_DATATYPE_OP_INSTANCE_FUNC("argmin-fix", ArgMin);
