...
const vector<string> Max<DType>::ITName = {
    "input",
};

// constructor and deconstructor
template <typename DType>
Max<DType>::Max(const xir::Subgraph* subg, const xir::Op* op, IMapTBs_t inputs,
                CPUTBPtr_t output)
...
void Max<DType>::run() {
  for (auto i = 0; i < fmap_o_.num(); i++) {
    ...
    data_out_[i] = std::max(data_ia_[pos_a], data_ib_[pos_b]);
  }
}

template <typename DType>
void Max<DType>::print_param() {
...
template <typename DType>
uint64_t Max<DType>::get_workload() {
  return fmap_o_.num();
}

template <typename DType>
void Max<DType>::max_align_dim() {
...
INSTANTIATE_TPCLASS(Max);
REG_OP_INSTANCE_FUNC("max", Max);
