...
class UnaryTanh : public UnaryBase<DType> {
public:
  UnaryTanh(const xir::Subgraph* subg, const xir::Op* op,
      IMapTBs_t inputs, CPUTBPtr_t output)
    : UnaryBase<DType>(subg, op, inputs, output) {
      UnaryBase<DType>::unary_type_ = UnaryBase<DType>::TANH;
  }
  virtual ~UnaryTanh() = default;
