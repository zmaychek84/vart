...
const vector<string> Min<DType>::ITName = {
    "input",
};

// constructor and deconstructor
template <typename DType>
Min<DType>::Min(const xir::Subgraph* subg, const xir::Op* op, IMapTBs_t inputs,
                CPUTBPtr_t output)
...
void Min<DType>::run() {
  for (auto i = 0; i < fmap_o_.num(); i++) {
    ...
    data_out_[i] = std::min(data_ia_[pos_a], data_ib_[pos_b]);
  }
}

template <typename DType>
void Min<DType>::print_param() {
...
template <typename DType>
uint64_t Min<DType>::get_workload() {
  return fmap_o_.num();
}

template <typename DType>
void Min<DType>::min_align_dim() {
...
INSTANTIATE_TPCLASS(Min);
REG_OP_INSTANCE_FUNC("min", Min);
