...
ReduceBase<DType>::ReduceBase(const xir::Subgraph* subg, const xir::Op* op,
  ...
  GET_INPUT_ANY_DIMX_FMAP(fmap_i_, input);
...
extern template string Vec2Str<string>(const vector<string>& v,
                                       const string& split);

extern template string Vec2Str<bool>(const vector<bool>& v,
                                     const string& split);
