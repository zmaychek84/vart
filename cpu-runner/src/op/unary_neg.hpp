...
class UnaryNeg : public UnaryBase<DType> {
public:
  UnaryNeg(const xir::Subgraph* subg, const xir::Op* op,
      IMapTBs_t inputs, CPUTBPtr_t output)
    : UnaryBase<DType>(subg, op, inputs, output) {
      UnaryBase<DType>::unary_type_ = UnaryBase<DType>::NEG;
  }
  virtual ~UnaryNeg() = default;
