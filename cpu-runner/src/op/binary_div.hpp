...
class BinaryDiv : public BinaryBase<DTypeIn0, DTypeIn1, DTypeOut> {
 public:
  BinaryDiv(const xir::Subgraph* subg, const xir::Op* op, IMapTBs_t inputs,
            CPUTBPtr_t output)
      : BinaryBase<DTypeIn0, DTypeIn1, DTypeOut>(subg, op, inputs, output) {
    BinaryBase<DTypeIn0, DTypeIn1, DTypeOut>::binary_type_ =
        BinaryBase<DTypeIn0, DTypeIn1, DTypeOut>::DIV;
  }
  virtual ~BinaryDiv() = default;
