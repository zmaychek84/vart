...
#include <cmath>
#include "unary_sigmoid_fix.hpp"
namespace vart {
namespace cpu {

template <typename DType>
UnarySigmoidFix<DType>::UnarySigmoidFix(const xir::Subgraph* subg, const xir::Op* op,
...
template <typename DType>
void UnarySigmoidFix<DType>::print_param() {
...
template <typename DType>
void UnarySigmoidFix<DType>::eltwise(std::uint32_t start_index,
  ...
  for (auto pos_iter = start_index; pos_iter < end_index; pos_iter++) {
    ...
    tmp = 1.0 / (1.0 + std::exp(-tmp));
    tmp = tmp / (float)pow(2.0f, shift_write_);
    data_out_[pos_iter] =
...
INSTANTIATE_TPCLASS(UnarySigmoidFix);
REG_OP_INSTANCE_FUNC("sigmoid-fix", UnarySigmoidFix);
