...
// this implementation has many many problems.
...
void Const<DType>::process() {
  if (data_type_ == xir::DataType::FLOAT) {
  ...
  } else if (data_type_ == xir::DataType::BFLOAT) {
    if (bit_width_ == 16) {
      for (auto i = 0; i < data_num_; i++) {
        data_out_ptr_[i] = ((xir::bfloat16_t*)(data_in_ptr_))[i];
      }
    } else {
      UNI_LOG_CHECK(0, VART_FUNC_NOT_SUPPORT)
          << "Unsupport bfloat" << bit_width_;
    }
  } else {
    if ((data_type_ == xir::DataType::XINT ||
    ...
    } else if ((data_type_ == xir::DataType::XINT ||
                data_type_ == xir::DataType::INT) &&
               bit_width_ == 16) {
      for (auto i = 0; i < data_num_; i++) {
        data_out_ptr_[i] = ((std::int16_t*)(data_in_ptr_))[i];
      }
    } else if ((data_type_ == xir::DataType::INT ||
                data_type_ == xir::DataType::XINT) &&
               bit_width_ == 64) {
      for (auto i = 0; i < data_num_; i++) {
        data_out_ptr_[i] =
            static_cast<DType>(((std::int64_t*)(data_in_ptr_))[i]);
      }
    } else if ((data_type_ == xir::DataType::INT) && bit_width_ == 32) {
      for (auto i = 0; i < data_num_; i++) {
        data_out_ptr_[i] =
            static_cast<DType>(((std::int32_t*)(data_in_ptr_))[i]);
      }
    } else if ((data_type_ == xir::DataType::XUINT ||
                data_type_ == xir::DataType::UINT) &&
               bit_width_ == 8) {
      for (auto i = 0; i < data_num_; i++) {
        data_out_ptr_[i] =
            static_cast<DType>(((std::uint8_t*)(data_in_ptr_))[i]);
      }
    } else if ((data_type_ == xir::DataType::XUINT ||
                data_type_ == xir::DataType::UINT) &&
               bit_width_ == 16) {
      for (auto i = 0; i < data_num_; i++) {
        data_out_ptr_[i] =
            static_cast<DType>(((std::uint16_t*)(data_in_ptr_))[i]);
      }
    } else {
      process_normal();
    }
