...
class AlignBuf {
public:
  AlignBuf(uint64_t size, uint64_t align)
  ...
  static unique_ptr<AlignBuf> make(uint64_t size, uint64_t align) {
...
private:
  ...
  struct InternalStatistics {
    uint64_t allocate_cnt_{0};
    uint64_t release_cnt_{0};
    uint64_t new_cnt_{0};
    uint64_t hit_cnt_{0};
    uint64_t err_cnt_{0};
  };
...
public:
  ...
  void* allocate(uint64_t size, uint64_t align = alignof(std::max_align_t)) {
