...
std::vector<int32_t> CPUTensorBuffer::get_stride(const xir::Tensor* tensor,
                                                 bool ignore_def) {
...
#define COPY_DATA_IN_BY_TYPE(inner_type, outer_type)                           \
  do {                                                                         \
    ...
    for (auto index = 0U; index < data_num_; ++index) {                        \
      ptr_inner[index] = static_cast<inner_type>(ptr_outer[index]);            \
    }                                                                          \
  } while (0);
  ...
  } else if (dtype_ == xir::DataType::BFLOAT) {
    if (bit_width_ == 16) {
      COPY_DATA_IN_BY_TYPE(float, xir::bfloat16_t);
    } else {
      UNI_LOG_FATAL(VART_NOT_SUPPORT)
          << "Not supported bit width " << bit_width_;
    }
...
clone_as_host_flat_tensor_buffer(CPUTensorBuffer* buffer) {
  ...
  auto shape = tensor->get_shape();
  auto strides = std::vector<int32_t>(shape.size(), 0);
...
#define COPY_DATA_OUT_BY_TYPE(inner_type, outer_type)                          \
  do {                                                                         \
    ...
    auto* ptr_inner = reinterpret_cast<inner_type*>(data_ptr_);                \
    for (auto index = 0U; index < data_num_; ++index) {                        \
      ptr_outer[index] = static_cast<outer_type>(ptr_inner[index]);            \
    }                                                                          \
  } while (0);
  ...
  if (dtype_ == xir::DataType::INT) {
    ...
    if (bit_width_ == 16) {
      do {
        uint16_t* src = (uint16_t*)data_ptr_;
        uint16_t* dst = (uint16_t*)ptr;
        for (auto index = 0U; index < data_num_; ++index) {
          dst[index] = src[2 * index + 1];
          if (src[2 * index] != 0) {
            UNI_LOG_WARNING << "output should be bf16 data , index = " << index;
          }
        }
      } while (0);
    } else {
...
void CPUTensorBuffer::save_bin() {
  ...
  if (save_bin) {
    ...
    std::replace_if(
        tensor_name.begin(), tensor_name.end(),
...
void CPUTensorBuffer::internal_save_bin(const string& fname) {
  ...
  if (dtype_ == xir::DataType::FLOAT) {
    ...
    internal_save_bfloat16_bin(normal_fname);
  } else {
    if (bit_width_ == 4) {
      ...
      internal_save_64bit_bin(normal_fname);
    }
  }
}
...
void CPUTensorBuffer::internal_save_bfloat16_bin(const string& fname) {
  if (dtype_ != xir::DataType::BFLOAT) return;
  vector<uint16_t> save_buf(data_num_);
  uint32_t* p = reinterpret_cast<uint32_t*>(data_ptr_);
  for (uint32_t i = 0; i < data_num_; i++) {
    save_buf.data()[i] = static_cast<uint16_t>(p[i]>>16);
  }
  SaveBin(fname, (const char*)save_buf.data(), save_buf.size() * 2);
}
...
void CPUTensorBuffer::internal_save_4bit_bin(const string& fname) {
  ...
  if (if_signed_) {
    ...
    dt_2_signed4bit<int32_t>(p, save_buf.data(), data_num_,
...
void CPUTensorBuffer::internal_save_64bit_bin(const string& fname) {
  if (bit_width_ != 64) return;
  std::vector<int64_t> d;
  for (unsigned i = 0; i < get_data_num(); i++)
    d.push_back(static_cast<int64_t>(((int32_t*)data_ptr_)[i]));

  SaveBin(fname, (const char*)d.data(),
          d.size() * sizeof(int64_t) / sizeof(char));
}
