...
static long cpu_num = 4;

long get_cpu_num() { return cpu_num; }

void set_cpu_num(int cpu_num_param) {
  const int max_cpu_num = (int)std::thread::hardware_concurrency();
  cpu_num = (cpu_num_param < 1)             ? 1
            : (cpu_num_param > max_cpu_num) ? max_cpu_num
                                            : cpu_num_param;
  UNI_LOG_INFO << "Set cpu_num = " << cpu_num;
}
...
std::string get_data_type_str(const xir::Tensor* xir_tensor) {
  ...
  if (xir_dtype == xir::DataType::INT) {
    ...
    s = "bfloat_" + to_string(bit_width) + "bit";
  } else {
