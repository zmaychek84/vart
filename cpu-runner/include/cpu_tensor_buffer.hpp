...
class CPUTensorBuffer : public TensorBuffer {
 ...
 public:
  ...
  std::vector<std::int32_t> get_stride(const xir::Tensor* tensor,
 ...
 private:
  ...
  void internal_save_bfloat16_bin(const string& fname);
  ...
  void internal_save_64bit_bin(const string& fname);
 ...
 private:
  ...
  // NOTE: Special handle for fix op, if we want to dump
  // fix op's middle result, save fix data here is the best choice.
  uint32_t fix_data_num_=0;
  uint32_t fix_data_size_=0;
