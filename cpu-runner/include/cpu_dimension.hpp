...
// NOTE: FMap_t in cpu_types.hpp is a special shape which has 4 dimensions,
...
public:
  ...
  inline dim_t coord2pos(const coord_t& coord) const {
    dim_t pos = 0;
    for (auto i = 0U; i < std::min(coord.size(), cod_.size()); i++) {
      pos += coord[i] * cod_[i];
    }

    return pos;
  }
  // this is another version of pos2coord, it doesn't
  // allocate a vector interally, which avoids small-size
  // frequent fragment memory allocation.
  inline void pos2coord(dim_t pos, coord_t& coord) const {
    auto remainder = pos;
    for (auto i = 0; i < ndims(); i++) {
      coord[i] = (remainder / cod_[i]);
