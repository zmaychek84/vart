...
MODEL_TYPE XrnnController::get_model_type(const std::string& model_name) {
  return model_type_map.count(model_name) == 1 ? model_type_map[model_name]
                                               : UNKOWN;
}
...
std::string XrnnController::get_model_name(MODEL_TYPE model_type) {
  ...
  for (iter = model_type_map.begin(); iter != model_type_map.end(); iter++) {
    if (iter->second == model_type) return iter->first;
  }
  return std::string("");
}

std::vector<uint32_t> XrnnController::get_reg_data(int frame,
                                                   int thread_index) {
  ...
  std::string reg_name = board + "_" + model + "_cu" + std::to_string(idx_);
  CHECK(reg_init_map.count(reg_name) == 1)
      << "Can Not Find Reg Init data " << reg_name;
  ...
  if (board == "u50") {
    ...
    regs_array[reg_init_data[reg_init_data.size() - 2].addr / 4] =
        reg_init_data[reg_init_data.size() - 2].value +
        (thread_index * THREAD_STEP);
    regs_array[reg_init_data[reg_init_data.size() - 1].addr / 4] =
        reg_init_data[reg_init_data.size() - 1].value +
        (thread_index * THREAD_STEP);
  } else if (board == "u25") {
    ...
    if (model_type_ == OPENIE) {
      ...
      for (int i = 0; i < 16; i++) {
        if (i % 2 == 0)
          regs_array[reg_init_data[start + i].addr / 4] =
              ADDR(RESL) + (frame - 1) * reg_init_data[43].value;
        else
          regs_array[reg_init_data[start + i].addr / 4] =
              ADDR(VECTOR) + (frame - 1) * reg_init_data[43].value;
      }
      regs_array[reg_init_data[start + 7].addr / 4] = ADDR(VECTOR);
      regs_array[reg_init_data[start + 15].addr / 4] = ADDR(VECTOR);
    }

    unsigned i = model_type_ == OPENIE ? reg_init_data.size() - 24
                                       : reg_init_data.size() - 3;
...
int XrnnController::get_batch_size() { return batch_; }
...
XrnnController::XrnnController(size_t device_core_id,
  ...
  if (kernel_name.find("slr") != kernel_name.npos)
    batch_ = device_core_id == 0 ? 3 : 4;
  else
    batch_ = 1;
}

void XrnnController::run(char* in, uint64_t isize, char* out, uint64_t osize,
                         int batch, int frame, int thread_index) {
  CHECK(batch <= batch_) << "Invalid Batch Size";

  LOG_IF(INFO, ENV_PARAM(DEBUG_XRNN_CONTROLLER)) << "device_core_id " << idx_;
  ...
  xrt_cu_->run(
      ...
      [=]() -> void {
        LOG_IF(INFO, ENV_PARAM(DEBUG_XRNN_CONTROLLER))
            << "xrnn excute done! "
            << "device_core_id = " << idx_ << " thread = " << thread_index;
        ...
        if (model_type_ == OPENIE) {
          ...
          for (auto i = 0; i < batch; i++) {
            ...
            LOG_IF(INFO, ENV_PARAM(DEBUG_XRNN_CONTROLLER))
                << "result output addr: " << std::hex
                << (base_addr + ADDR(RESL) + thread_index * THREAD_STEP);
            memory_->download(
                (void*)(out + i * osize),
...
void XrnnController::update(int frame, ModelConfig* mc, uint32_t* p_ddr,
                            size_t size) {
  ...
  LOG_IF(INFO, ENV_PARAM(DEBUG_XRNN_CONTROLLER)) << "layers: " << layers;
    ...
    for (int b = 0; b < batch_; b++) {
      ...
      if (i % 2 == 0) {
        ...
        p_ddr[(ddr_regs_ptr + (reg_load1 + reg_step * b)) / 4] =
            batch_base + ADDR(RESL);
        p_ddr[(ddr_regs_ptr + (reg_save0 + reg_step * b)) / 4] =
            batch_base + ADDR(RESL);
      } else {
        p_ddr[(ddr_regs_ptr + (reg_load0 + reg_step * b)) / 4] =
            batch_base + ADDR(RESL) + offset_load0;
        p_ddr[(ddr_regs_ptr + (reg_load1 + reg_step * b)) / 4] =
            batch_base + ADDR(VECTOR);
        p_ddr[(ddr_regs_ptr + (reg_save0 + reg_step * b)) / 4] =
            batch_base + ADDR(VECTOR);
      }
    }
