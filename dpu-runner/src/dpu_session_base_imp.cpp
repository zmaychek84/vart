...
DEF_ENV_PARAM(XLNX_MAX_USER_BATCH, "0");
namespace vart {
namespace dpu {
...
size_t DpuSessionBaseImp::my_get_device_core_id(size_t cu_size,
  ...
  if (attrs) {
    ...
    if (attrs->has_attr("__device_id__")) {
      UNI_LOG_CHECK(device_id == attrs->get_attr<size_t>("__device_id__"),
    ...
    if (!attrs_->has_attr("__batch__")) {
      attrs_->set_attr<size_t>("__batch__", get_max_user_batch(device_core_id));
    }
  } else {
    session_count = session_count + 1u;
...
std::vector<my_tensor_t> DpuSessionBaseImp::init_tensors(
  ...
  auto user_batch = get_max_user_batch(get_device_core_id());

  std::transform(
      tensor_names.begin(), tensor_names.end(), std::back_inserter(ret),
      [&graph, subgraph, user_batch, check_stride](const auto& tensor_name) {
...
// for large model, the BO allocation size may exceed CMA size
// short term: use env to control the max user batch, then the idle engine will
//             get the 0xF address, so for xv2, dpu can skip ddr fetch.
// note: for the other architectures, dpu don't support ddr skip, but since this
//       env is used as a backdoor, the arch distinguish is deferred here
size_t DpuSessionBaseImp::get_max_user_batch(size_t device_core_id) const {
  size_t engine_num = const_cast<DpuSessionBaseImp*>(this)
                          ->get_dpu_controller()
                          ->get_batch_size(device_core_id);

  if (ENV_PARAM(XLNX_MAX_USER_BATCH) > 0) {
    size_t user_batch = ENV_PARAM(XLNX_MAX_USER_BATCH);
    CHECK(user_batch <= engine_num)
        << "XLNX_MAX_USER_BATCH must not exceed engine number " << engine_num;
    return user_batch;
  }
